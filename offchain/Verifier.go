...
func (vr *Verifier) VeriProof(challenges []Challenge, proofObject ProofObject, id string) bool {
	...
	miu := vr.keys.pairing.NewZr().SetBytes(base64StringToElementBytes(miuString))
	wString := proofObject.R
	r := vr.keys.pairing.NewG2().SetBytes(base64StringToElementBytes(wString))

	hashMulString := proofObject.HashMul
	hashMul := vr.keys.pairing.NewG1().SetBytes(base64StringToElementBytes(hashMulString))

	//miu := proof.Miu
	//	r := proof.R
	//hashMul := proof.HashMul
	tags := readTags(vr.keys, id, challenges) // 相当于读取tags
